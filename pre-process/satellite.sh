...
do
	...
	do
		...
		do
		    ...
		    if [ "$n" -gt 0 ]
		    then
			domain=satellite.pddl.${num_poss_pre}p_${num_poss_add}a_${num_poss_del}d.${dom_count}
			problem=pfile$prob_count
			input=${domain}_${problem}.out
			output=satellite@default.csv
			echo "Processing" ${input} " --> output to " ${output} 
			./read-default ./default-satellite-raw/ ${input} ./ ${output} ${domain} ${problem}
		    fi
		done
	    done
	done
